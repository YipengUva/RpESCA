...
## test varExp_Gaussian function
# fix a data set into the package and use it to test the function

## test log10_seq function
from <- 1
to <- 500
length.out <- 30
expect_true(all(10^(seq(log10(from),log10(to),length.out=length.out))==
      log10_seq(from, to, length.out)))

## test obj_logistic function
