...
#'
#' @export
dataSimu_group_sparse <- function(n,ds,
                                  dataTypes='GGG',
                                  noises=rep(1,3),
  ...
  S_simu <- cbind(matrix(rep(c(1,1,1),Rgc),3,Rgc),
                  matrix(rep(c(1,1,0),Rlc[1]),3,Rlc[1]),
                  matrix(rep(c(1,0,1),Rlc[2]),3,Rlc[2]),
                  matrix(rep(c(0,1,1),Rlc[3]),3,Rlc[3]),
                  matrix(rep(c(1,0,0),Rd[1]),3,Rd[1]),
                  matrix(rep(c(0,1,0),Rd[2]),3,Rd[2]),
                  matrix(rep(c(0,0,1),Rd[3]),3,Rd[3]))
  ...
  for(i in 1:3){
    ...
    if(dataTypes[i]=='G'){
      ...
      X_tmp <- sign(inver_logit(Theta_simu[,columns_Xi]) - matrix(stats::runif((n*ds[i])),n,ds[i]))
      tmp_Xi <- 0.5*(X_tmp+1)
    }
  ...
  # noise levels are assumed to be equal here
...
test_dataSimu_group <- function(n, ds, dataTypes){
  ...
  for(i in 1:length(ds)){
    ...
    if(all(unique_values %in% c(1,0))){dataTypes_test[i] <- 'B'}
  }
  ...
  test_ortho <- all.equal(diag(UtU),rep(1, sumR)) &
    all.equal(UtU- diag(diag(UtU)), matrix(data=0,sumR,sumR))
  ...
  for(i in 1:length(SNRs_object)){
    ...
    Theta_factors <- simulatedData$U_simu[,index_factors]%*%
      diag(simulatedData$D_simu[index_factors])%*%
