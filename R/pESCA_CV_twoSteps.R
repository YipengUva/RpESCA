...
#'
#' @export
pESCA_CV_twoSteps <-function(dataSets, dataTypes,
                             lambdas_g, lambdas_b,
  ...
  # remove the first two points, because usually they are not convergenced.
  # Therefore have relative low CV error
  ...
  # fix lambda_b_opt, optimize lambda_g
  inits <- list() # save the parameters during the model selection
  for(j in 1:nTries_g){
